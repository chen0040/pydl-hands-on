...
def maybe_build_vocab(reuters_dir, vocab_file):
    ...
    if os.path.exists(vocab_file):
        fvoc = open(vocab_file, "rt", encoding='utf8', errors='ignore')
        for line in fvoc:
...
DATA_DIR = "../../../data"
VERY_LARGE_DATA_DIR = '../../../data'

REUTERS_DIR = os.path.join(VERY_LARGE_DATA_DIR, "reuters-21578")
VOCAB_FILE = os.path.join(VERY_LARGE_DATA_DIR, "reuters-21528-vocab.txt")
VOCAB_SIZE = 5000

vocab = maybe_build_vocab(REUTERS_DIR, VOCAB_FILE)

ftext = open(os.path.join(DATA_DIR, "reuters-21528-text.tsv"), "wt")
ftags = open(os.path.join(DATA_DIR, "reuters-21528-tags.tsv"), "wt")
num_read = 0
for doc in stream_reuters_documents(REUTERS_DIR):
