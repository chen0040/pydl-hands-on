        ...
        for index in range(int(X_train.shape[0] / BATCH_SIZE)):
            ...
            if index % 20 == 0:
                ...
                Image.fromarray(image.astype(np.uint8)).save(
                    get_path(str(epoch) + "_" + str(index) + ".png"))
            ...
            if index % 20 == 0:
                ...
                Image.fromarray(image_real.astype(np.uint8)).save(
                    get_path("target" + str(epoch) + "_" + str(index) + ".png"))
            ...
            if index % 10 == 9:
                generator.save_weights(get_path('generator'), True)
                discriminator.save_weights(get_path('discriminator'), True)


def generate(BATCH_SIZE, nice=False):
    generator = generator_model()
    generator.compile(loss='binary_crossentropy', optimizer="SGD")
    generator.load_weights(get_path('generator'))
    if nice:
        discriminator = discriminator_model()
        discriminator.compile(loss='binary_crossentropy', optimizer="SGD")
        discriminator.load_weights(get_path('discriminator'))
